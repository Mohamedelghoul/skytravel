...
int verifier_code(char C[],int typeprestation,int type)
{
   if(type==0)
   {
	...
	if(f !=NULL)
	{
		while(fscanf(f,"%s",E.id_devis)!=EOF)
		{
	...
	{
		while(fscanf(f,"%s",V.id_devis)!=EOF)
		{
   ...
   if(type==1)
   {
	...
	if(f !=NULL)
	{
		while(fscanf(f,"%s",H.id_devis)!=EOF)
		{
			if (strcmp(C,H.id_devis)==0)
				return (1);
		}
...
void ajouter_facture(int typeprestation)
{
	if (typeprestation==0)
	{
			Dhebergement H;
			...
			FILE *f1=NULL;
			f=fopen("devishebergement.txt","a+");
			f1=fopen("hebergementreserve.txt","a+");
			if(f1!=NULL)
			{
				if(f!=NULL)
				{
					while(fscanf(f1,"%s %s %s %s %s %s %s %s %s %s %s %s \n",H.identifiant,H.id_reservation,H.id_hebergement,H.depart,H.arrivee,H.destination,H.nom_hotel,H.personne,H.enfants,H.chambres,H.pension,H.montant)!=EOF)
					{
						do
						{
						sprintf(H.id_devis,"%d",code_devis(0)+nb);
						nb++;
						}
						while(verifier_code(H.id_devis,0,0)==1);
			...
			fclose(f);
			fclose(f1);
			f=fopen("hebergementreserve.txt","w");
			fclose(f);
	}
	else if (typeprestation==1)
		{
			...
			FILE *f1=NULL;
			f=fopen("devisexcursion.txt","a+");
			f1=fopen("excursionsreservees.txt","a+");
			if(f1!=NULL)
			{
				if(f!=NULL)
				{
					while(fscanf(f1,"%s %s %s %s %s %s %s %s %s %s \n",E.identifiant,E.id_reservation,E.id_excursion,E.depart,E.arrivee,E.d.jour,E.d.mois,E.d.annee,E.personne,E.montant)!=EOF)
					{
						do
						{
						sprintf(E.id_devis,"%d",code_devis(1)+nb);
						nb++;
						}
						while(verifier_code(E.id_devis,1,0)==1);
			...
			fclose(f);
			fclose(f1);
			f=fopen("excursionsreservees.txt","w");
			fclose(f);
			...
			if(f1!=NULL)
			{
				if(f!=NULL)
				{
					while(fscanf(f1,"%s %s %s %s %s %s %s %s %s %s %s %s %s \n",V.identifiant,V.id_reservation,V.id_vol,V.depart.jour,V.depart.mois,V.depart.annee,V.arrivee.jour,V.arrivee.mois,V.arrivee.annee,V.vdepart,V.vdestination,V.personne,V.montant)!=EOF)
					{
						//do
						//{
						sprintf(V.id_devis,"%d",code_devis(2)+nb);
						nb++;
						//}
						//while(verifier_code(V.id_devis,2,0)==1);
