                                        ...
                                        gpointer         user_data)
{
	...
	comboboxidMADH50=lookup_widget(WModifier_CDH,"comboboxidMADH");
	n=remplir_combo_hebergement(tab);
	for(i=0;i<n;i++)
	{
		gtk_combo_box_append_text(GTK_COMBO_BOX (comboboxidMADH50),_(tab[i].id_devis));
	}
}
                                        ...
                                        gpointer         user_data)
{
	...
	if(f!=NULL)
	{
		while(fscanf(f,"%s %s \n",code1,promos)!=EOF)
		{
			if(strcmp(code,code1)==0)
			{
				...
				sscanf(H.montant,"%d",&M);
				sscanf(promos,"%d",&P);
				montantf=M*P;
				sprintf(Montant,"%d",montantf);
				fprintf(f2,"%s %s %s %s %s %s %s %s %s %s %s %s %s \n",H.id_devis,H.identifiant,H.id_reservation,H.id_hebergement,H.depart,H.arrivee,H.destination,H.nom_hotel,H.personne,H.enfants,H.chambres,H.pension,Montant);
				}
			}
			else
			{
	...
	msg=lookup_widget(objet,"introuvablecodeCDH");
	if(b==1)
	{
	...
	{
		gtk_label_set_text(GTK_LABEL(msg),"Code promos non trouvé !");
	}
                                        ...
                                        gpointer         user_data)
{
	...
	if(f!=NULL)
	{
		while(fscanf(f,"%s %s %s %s %s %s %s %s %s %s %s %s %s \n",H.id_devis,H.identifiant,H.id_reservation,H.id_hebergement,H.depart,H.arrivee,H.destination,H.nom_hotel,H.personne,H.enfants,H.chambres,H.pension,H.montant)!=EOF)
		{
			...
			{
				supprimer_devis(id,0);
				sscanf(H1.montant,"%d",&montant);
				montantf=montant*R1;
				sprintf(montantF,"%d",montantf);
				fprintf(f,"%s %s %s %s %s %s %s %s %s %s %s %s %d \n",H.id_devis,H.identifiant,H.id_reservation,H.id_hebergement,H.depart,H.arrivee,H.destination,H.nom_hotel,H.personne,H.enfants,H.chambres,H.pension,montantf);
                                        ...
                                        gpointer         user_data)
{
	...
	strcpy(mode,gtk_combo_box_get_active_text(GTK_COMBO_BOX(comboboxmodepayement)));
		input1=lookup_widget(objet,"iddevispayement");
	input1=lookup_widget(objet,"iddevispayement");
	strcpy(code,gtk_entry_get_text(GTK_ENTRY(input1)));*/
	if((strcmp(mode,"Espece")==0))
	{
		...
		gtk_widget_destroy(mode_payement);
		comboboxCCDH=lookup_widget(WContinuer_CDH,"comboboxCCDH");
		strcpy(mode,gtk_combo_box_get_active_text(GTK_COMBO_BOX(comboboxCCDH)));
	...
	//gtk_entry_set_text(GTK_ENTRY(iddevispayement),id); 
