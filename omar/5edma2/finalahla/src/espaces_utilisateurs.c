...
int remplir_combobox_numero(char T[50][20])
{
	int n=0;
	utilisateur u;
	FILE* f=NULL;
	...
	if(f!=NULL)
	{
		while(fscanf(f,"%s %s %s %s %d %d %d %s %s \n",u.identifiant,u.mot_de_passe,u.nom,u.prenom,&u.naissance.jour,&u.naissance.mois,&u.naissance.annee,u.cin,u.role)!=EOF)
		{
				if ((strcmp(u.role,"employee")==0) || (strcmp(u.role,"admin")==0))
				{
					strcpy(T[n],u.identifiant);
					n++;
				}
		}
	}
...
int remplir_combobox_numero_1(char T[50][20])
{
	int n=0;
	utilisateur u;
	FILE* f=NULL;
