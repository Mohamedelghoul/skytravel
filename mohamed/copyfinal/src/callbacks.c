                                        ...
                                        gpointer         user_data)
{
	...
	if(f!=NULL)
	{
		while(fscanf(f,"%s %s %s %s %s %s %s %s %s %s %s %s %s \n",H.id_devis,H.identifiant,H.id_reservation,H.id_hebergement,H.depart,H.arrivee,H.destination,H.nom_hotel,H.personne,H.enfants,H.chambres,H.pension,H.montant)!=EOF)			if(strcmp(H.id_devis,id1)==0)
			{
				strcpy(H.montant,A);
				entrymontantpayementF=lookup_widget(objet,"entrymontantpayementF");
				gtk_entry_set_text(GTK_ENTRY(entrymontantpayementF),A);
                                        ...
                                        gpointer         user_data)
{
	...
	FILE *f1;
	FILE *f2;
	f1=fopen("devishebergement","a+");
	f2=fopen("facturehebergement","w");
	f=fopen("actual_user.txt","r");
	fscanf(f,"%s",identifiant);
	strcpy(idC,identifiant); 
	Payement=lookup_widget(objet,"Payement");
